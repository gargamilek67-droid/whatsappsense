package opm.whatsapp.api.util.discord;
...
public class DiscordManager {
    ...
    public boolean start(boolean isInitialStart) {
        ...
        if (reachedMaxAttempts) {
            WhatsApp.LOGGER.warn("Not starting Discord RPC - maximum restart attempts already reached");
            return false;
        }
        ...
        if (success) {
            enabled = true;
            updatePresence();
            WhatsApp.LOGGER.info("Discord RPC started successfully");
            wasReconnecting = currentRestartAttempts > 0;
        } else {
            WhatsApp.LOGGER.error("Failed to start Discord RPC");
    ...
    public void stop() {
        ...
        cancelRestartTask();
        DiscordIPC.stop();
        WhatsApp.LOGGER.info("Discord RPC stopped");
    }
    ...
    private void handleError(int code, String message) {
        WhatsApp.LOGGER.error("Discord RPC error {}: {}", code, message);
        enabled = false;

        if (reachedMaxAttempts) {
            WhatsApp.LOGGER.warn("Maximum Discord RPC restart attempts already reached. Not retrying.");
            return;
        }
        ...
        if (autoRestart && (maxRestartAttempts < 0 || currentRestartAttempts < maxRestartAttempts)) {
            ...
            WhatsApp.LOGGER.warn("Maximum Discord RPC restart attempts reached ({})", maxRestartAttempts);
        }
    }
    ...
    private void scheduleRestart() {
        ...
        WhatsApp.LOGGER.info("Scheduling Discord RPC restart in {} seconds (attempt {}/{})",
                restartDelay, currentRestartAttempts, maxRestartAttempts < 0 ? "unlimited" : maxRestartAttempts);

        restartTask = executor.schedule(() -> {
            WhatsApp.LOGGER.info("Attempting to restart Discord RPC");
            DiscordIPC.stop();
            start(false);
    ...
    private void onReady() {
        WhatsApp.LOGGER.info("Discord RPC connected as {}", DiscordIPC.getUser().username);

        // Only reset retry state if this was a successful reconnection after failures
        if (wasReconnecting) {
            WhatsApp.LOGGER.info("Discord RPC reconnected successfully after {} attempts", currentRestartAttempts);
            resetRetryState();
        }
    }
