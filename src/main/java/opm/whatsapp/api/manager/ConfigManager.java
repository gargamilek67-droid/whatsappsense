...
import opm.whatsapp.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "whatsapp/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        WhatsApp.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    WhatsApp.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("whatsapp").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("whatsapp/" + name + "/"))) {
            this.config = "whatsapp/" + name + "/";
        } else {
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("whatsapp").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("whatsapp/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        WhatsApp.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("whatsapp/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("whatsapp", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(WhatsApp.moduleManager.modules);
        this.features.add(WhatsApp.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        WhatsApp.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            WhatsApp.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
