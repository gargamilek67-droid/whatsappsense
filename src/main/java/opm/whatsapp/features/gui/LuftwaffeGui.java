package opm.whatsapp.features.gui;

import opm.whatsapp.WhatsApp;
import opm.whatsapp.features.Feature;
import opm.whatsapp.features.gui.components.Component;
import opm.whatsapp.features.gui.components.items.Item;
import opm.whatsapp.features.gui.components.items.buttons.ModuleButton;
...
public class LuftwaffeGui
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : WhatsApp.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    WhatsApp.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (hovered != null) {
            ...
            if (desc != null && !desc.isEmpty()) {
                int descWidth = WhatsApp.textManager.getStringWidth(desc) + 8;
                int descHeight = 12;
                int x = mouseX + 10;
                int y = mouseY + 10;
                net.minecraft.client.gui.Gui.drawRect(x, y, x + descWidth, y + descHeight, 0xAA000000);
                WhatsApp.textManager.drawStringWithShadow(desc, x + 4, y + 2, 0xFFFFFF);
            }
        }
    }
