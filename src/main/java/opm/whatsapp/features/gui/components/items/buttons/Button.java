        ...
        if (enabled && showBg) {
            RenderUtil.drawRect(
                    ...
                    !this.isHovering(mouseX, mouseY)
                            ? WhatsApp.colorManager.getColorWithAlpha(WhatsApp.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue())
                            : WhatsApp.colorManager.getColorWithAlpha(WhatsApp.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())
            );
        }

        WhatsApp.textManager.drawStringWithShadow(
                this.getName(),
                this.x + 2.3f,
