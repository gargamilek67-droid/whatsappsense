package opm.whatsapp.features.modules.misc;
...
import opm.whatsapp.api.event.events.PacketEvent1;
...
import opm.whatsapp.api.util.MathUtil;
import opm.whatsapp.api.util.NullUtils;
import opm.whatsapp.api.util.Timer;
import opm.whatsapp.api.event.events.EntityAddEvent;
...
public class Alerts
    ...
    public Alerts() {
        super("Notifications", "Combat info+", Category.MISC, true, false, false);
        INSTANCE = this;
    ...
    public void onPop(Entity entity) {
        ...
        if (this.pops.getValue().booleanValue()) {
            ...
            if (this.icehackStyle.getValue().booleanValue()) {
                Command.sendSilentMessage("" + ChatFormatting.DARK_RED + ChatFormatting.BOLD + "whatsapp" + ChatFormatting.GRAY + ChatFormatting.BOLD + ".cz" +  " " + (this.boldName.getValue() != false ? ChatFormatting.BOLD : "") + (isSelf ? "You" : name) + ChatFormatting.RESET + ChatFormatting.DARK_RED + (isSelf ? " have" : " has") + " popped" + (pops == 0 ? "." : (isSelf ? " your " : " their ") + ChatFormatting.GOLD + pops + MathUtil.getOrdinal(pops) + " totem."));
            } else {
                Command.sendSilentMessage("" + ChatFormatting.DARK_RED + ChatFormatting.BOLD + "whatsapp" + ChatFormatting.GRAY + ChatFormatting.BOLD + ".cz" + "" + (this.boldName.getValue() != false ? ChatFormatting.BOLD : "") + (isSelf ? "You" : name) + ChatFormatting.RESET + ChatFormatting.AQUA + (isSelf ? " have" : " has") + " popped" + (pops == 0 ? "." : (isSelf ? " your " : " their ") + ChatFormatting.BLUE + pops + MathUtil.getOrdinal(pops) + ChatFormatting.AQUA + " totem"));
            }
        }
    }
    ...
    public void onDeath(EntityPlayer player) {
        ...
        if (this.pops.getValue().booleanValue() && this.registry.containsKey((name = player.getName()))) {
            ...
            if (this.icehackStyle.getValue().booleanValue()) {
                Command.sendSilentMessage("" + ChatFormatting.DARK_RED + ChatFormatting.BOLD + "whatsapp" + ChatFormatting.GRAY + ChatFormatting.BOLD + ".cz" +  " " + ChatFormatting.DARK_AQUA + "" + (this.boldName.getValue() != false ? ChatFormatting.BOLD : "") + player.getName() + ChatFormatting.RESET + ChatFormatting.DARK_RED + " died after popping" + (pops == 0 ? "." : " their " + ChatFormatting.GOLD + pops + MathUtil.getOrdinal(pops) + " totem."));
            } else {
                Command.sendSilentMessage("" + ChatFormatting.DARK_RED + ChatFormatting.BOLD + "whatsapp" + ChatFormatting.GRAY + ChatFormatting.BOLD + ".cz" +  " " + ((this.boldName.getValue() != false ? ChatFormatting.BOLD : "") + player.getName() + ChatFormatting.RESET + ChatFormatting.AQUA + " died after popping" + (pops == 0 ? "." : " their " + ChatFormatting.BLUE + pops + MathUtil.getOrdinal(pops) + ChatFormatting.AQUA + " totem")));
            }
        }
    }
    ...
    public void onEntityAdd(EntityAddEvent event) {
        ...
        if (this.pearls.getValue().booleanValue() && event.getEntity() instanceof EntityEnderPearl && (kidWhoThrowPearl = Alerts.mc.world.getClosestPlayerToEntity(event.getEntity(), 3.0)) != null) {
            ...
            Command.sendSilentMessage("" + ChatFormatting.DARK_RED + ChatFormatting.BOLD + "whatsapp" + ChatFormatting.GRAY + ChatFormatting.BOLD + ".cz" +  " " + ("" + (FriendManager.getInstance().isFriend(kidWhoThrowPearl) ? ChatFormatting.AQUA : "") + (this.boldNamePearls.getValue() != false ? ChatFormatting.BOLD : "") + kidWhoThrowPearl.getDisplayName().getUnformattedText() + ChatFormatting.WHITE + " has thrown an ender pearl " + ChatFormatting.AQUA + facing + "!"));
        }
    }
            ...
            for (EntityPlayer player : Alerts.mc.world.playerEntities) {
                ...
                if (this.strengthDetect.getValue().booleanValue()) {
                    if (player.isPotionActive(MobEffects.STRENGTH) && !this.list.contains(player)) {
                        Command.sendSilentMessage("" + ChatFormatting.DARK_RED + ChatFormatting.BOLD + "whatsapp" + ChatFormatting.GRAY + ChatFormatting.BOLD + ".cz" +  " " + ("" + (FriendManager.INSTANCE.isFriend(player) || isSelf ? ChatFormatting.AQUA : ChatFormatting.DARK_AQUA) + (isSelf ? "You" : player.getDisplayName().getUnformattedText()) + ChatFormatting.GREEN + " now " + (isSelf ? "have " : "has ") + "Strength!"));
