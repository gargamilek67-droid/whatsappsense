...
public class AutoReply extends Module {
    ...
    public AutoReply() {
        super("AutoReply", "auto reply rbh", Module.Category.MISC, false, false, false);
    }
        ...
        if (!fullNullCheck() && !this.isDisabled()) {
            if (e.getPacket() instanceof SPacketChat) {
                ...
                if (unformatted.contains("says: ") || unformatted.contains("whispers: ")) {
                    ...
                    if (mc.player.getName().equals(ign) || !WhatsApp.friendManager.isFriend(ign)) {
                        return;
                    }
                    ...
                    if ((Boolean)this.coords.getValue() && msg.matches(".*(cord|coord|coords|cords|wya|where are u|where are you|where r u|where ru).*") && !msg.matches(".*(discord|record).*")) {
                        ...
                        mc.player.sendChatMessage("/msg " + ign + " My coordinates are X: " + x + " Y: " + (int)mc.player.posY + " Z: " + z + " in the " + dimension + " (Thanks to WhatsApp)");
                    }
                }
            }
        }
