...
public class HUD extends Module {
    ...
    public Setting<String> command = (new Setting("Command", "whatsapp"));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.waterMark.getValue().booleanValue()) {
            String string = this.command.getPlannedValue() + WhatsApp.MODVER;
        ...
        if (this.arrayList.getValue().booleanValue())
            if (this.renderingUp.getValue().booleanValue()) {
                if (this.renderingMode.getValue() == RenderingMode.ABC) {
                    for (int k = 0; k < WhatsApp.moduleManager.sortedModulesABC.size(); k++) {
                        String str = WhatsApp.moduleManager.sortedModulesABC.get(k);
                    ...
                    for (int k = 0; k < WhatsApp.moduleManager.sortedModules.size(); k++) {
                        Module module = WhatsApp.moduleManager.sortedModules.get(k);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            if (this.potions.getValue().booleanValue()) {
                ...
                for (PotionEffect potionEffect : effects) {
                    String str = WhatsApp.potionManager.getColoredPotionString(potionEffect);
                    i += 10;
            ...
            if (this.speed.getValue().booleanValue()) {
                String str = grayString + "Speed " + ChatFormatting.WHITE + WhatsApp.speedManager.getSpeedKpH() + " km/h";
                i += 10;
            ...
            if (this.tps.getValue().booleanValue()) {
                String str = grayString + "TPS " + ChatFormatting.WHITE + WhatsApp.serverManager.getTPS();
                i += 10;
                this.renderer.drawString(str, (width - this.renderer.getStringWidth(str) - 2), (height - 2 - i), (ClickGui.getInstance()).rainbow.getValue().booleanValue() ? (((ClickGui.getInstance()).rainbowModeA.getValue() == ClickGui.rainbowModeArray.Up) ? ColorUtil.rainbow(counter1[0] * (ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB()) : this.color, true);
                counter1[0] = counter1[0] + 1;
            }
            String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            String str1 = grayString + "Ping " + ChatFormatting.WHITE + WhatsApp.serverManager.getPing();
        ...
        String direction = this.direction.getValue().booleanValue() ? WhatsApp.rotationManager.getDirection4D(false) : "";
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (WhatsApp.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding " + MathUtil.round((float) WhatsApp.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
