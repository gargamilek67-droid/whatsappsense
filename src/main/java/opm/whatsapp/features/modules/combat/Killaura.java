package opm.whatsapp.features.modules.combat;

import opm.whatsapp.WhatsApp;
import opm.whatsapp.api.event.events.UpdateWalkingPlayerEvent;
import opm.whatsapp.features.modules.Module;
import opm.whatsapp.features.setting.Setting;
import opm.whatsapp.api.util.DamageUtil;
...
public class Killaura extends Module {
    ...
    private void doKillaura() {
        ...
        int wait = !this.delay.getValue().booleanValue() ? 0 : (int) (DamageUtil.getCooldownByWeapon(mc.player) * (this.tps.getValue().booleanValue() ? WhatsApp.serverManager.getTpsFactor() : 1.0F));
        ...
        if (target == null)
            return;
        if (this.rotate.getValue().booleanValue())
            WhatsApp.rotationManager.lookAtEntity(target);
